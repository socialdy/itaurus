...
const DropZoneContent: React.FC<{ maxSize: number; limit: number }> = ({
  ...
  <>
    ...
    <div className="tiptap-image-upload-content">
      <span className="tiptap-image-upload-text">
        <em>Klicken zum Hochladen</em> oder ziehen und ablegen
      </span>
      <span className="tiptap-image-upload-subtext">
        Maximal {limit} Datei{limit === 1 ? "" : "en"}, jeweils {maxSize / 1024 / 1024}MB.
      </span>
    </div>
  </>
)
...
export const ImageUploadNode: React.FC<NodeViewProps> = (props) => {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!files || files.length === 0) {
      extension.options.onError?.(new Error("Keine Datei ausgewählt"))
      return
    }
        ...
        <div className="tiptap-image-upload-previews">
          {fileItems.length > 1 && (
            <div className="tiptap-image-upload-header">
              <span>Lade {fileItems.length} Dateien hoch</span>
              <Button
                type="button"
                data-style="ghost"
                onClick={(e) => {
